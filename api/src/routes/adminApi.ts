...
adminApiRouter.post('/members/invite', demoGuard, async (c) => {
  const body = await c
    .req
    .json<{ name?: string; email?: string; role?: string }>()
    .catch(() => ({} as { name?: string; email?: string; role?: string }))
  ...
  // Create magic link (7-day expiry for invites)
  ...
  await recordAuthEvent(db, { event: 'link_requested', email: normalizedEmail, userId: id, linkType: 'invite', ...authReqContext(c) })
  try {
    c.executionCtx.waitUntil(sendMagicLink(normalizedEmail, url, c.env, 'invite', db, id).catch(console.error))
  } catch {
    sendMagicLink(normalizedEmail, url, c.env, 'invite', db, id).catch(console.error)
  }

  return c.json({ id, email: normalizedEmail }, 201)
})
