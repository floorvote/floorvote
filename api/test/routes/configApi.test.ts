...
describe('GET /config', () => {
  ...
  // ── GET /config must never rewrite the four AI-config keys ─────────────────
  //
  // A demo tenant's ai_context / relevance_question / tag_taxonomy / keywords come
  // from its seed (api/src/lib/demoSeeds/); a self-hosted tenant's come from the
  // admin UI. GET /config runs a bootstrap hook on every request, and the retired
  // preset version of that hook clobbered all four on a tenant's very first request
  // — silently, and on a demo only until the next reset. Nothing on this path may
  // write those keys again.
  ...
  async function expectSeedConfigIntact(db: ReturnType<typeof getDb>) {
  ...
  it('leaves a demo tenant\'s seed-written config alone', async () => {
    const db = getDb(env.DB)
    ...
    await expectSeedConfigIntact(db)
  })

  it('leaves a non-demo tenant\'s stored config alone', async () => {
    const db = getDb(env.DB)
    await seedWrittenAiConfig(db)
