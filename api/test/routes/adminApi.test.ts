...
describe('Admin Members API', () => {
  ...
  describe('POST /admin/members/invite', () => {
    it('returns 403 for a member user', async () => {
      const res = await app.request(
        '/api/admin/members/invite',
        {
          ...
          body: JSON.stringify({ name: 'New Person', email: 'new@example.com' }),
        },
        env,
      )
      expect(res.status).toBe(403)
    ...
    it('returns 400 for an invalid email address', async () => {
      const res = await app.request(
        '/api/admin/members/invite',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Cookie: adminCookie },
          body: JSON.stringify({ name: 'New Person', email: 'not-an-email' }),
        },
        env,
      )
      expect(res.status).toBe(400)
    ...
    it('returns 409 when email already exists', async () => {
      const res = await app.request(
        '/api/admin/members/invite',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Cookie: adminCookie },
          body: JSON.stringify({ name: 'Duplicate', email: 'member@example.com' }),
        },
        env,
      )
    ...
    it('returns 201 and creates user + magic link on success', async () => {
      const res = await app.request(
        '/api/admin/members/invite',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Cookie: adminCookie },
          body: JSON.stringify({ name: 'New Person', email: 'new@example.com', role: 'member' }),
        },
        env,
      )
      ...
      const user = await db.select().from(users).where(eq(users.email, 'new@example.com')).get()
      expect(user).toBeTruthy()
      expect(user!.role).toBe('member')
      expect(user!.invitedBy).toBe(adminId)

      const link = await db.select().from(magicLinks).where(eq(magicLinks.userId, body.id)).get()
    ...
    it('creates an admin user when role is admin', async () => {
      const res = await app.request(
        '/api/admin/members/invite',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Cookie: adminCookie },
          body: JSON.stringify({ name: 'New Admin', email: 'newadmin@example.com', role: 'admin' }),
        },
        env,
      )
      expect(res.status).toBe(201)
      const db = getDb(env.DB)
      const user = await db.select().from(users).where(eq(users.email, 'newadmin@example.com')).get()
      expect(user!.role).toBe('admin')
    })

    it('creates user with empty name when name is omitted', async () => {
      const res = await app.request(
        '/api/admin/members/invite',
        {
          method: 'POST',
...
describe('auth event logging — admin invite', () => {
  ...
  it('records link_requested (invite) when inviting a new member', async () => {
    const db = getDb(env.DB)
    const res = await app.request(
      '/api/admin/members/invite',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Cookie: adminCookie },
        body: JSON.stringify({ email: 'invitee@b.com', name: 'Invitee', role: 'member' }),
      },
      env,
    )
    expect(res.status).toBe(201)
    const rows = await db.select().from(authEvents).all()
    expect(rows.some(r => r.event === 'link_requested' && r.linkType === 'invite' && r.email === 'invitee@b.com')).toBe(true)
  })
