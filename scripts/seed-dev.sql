...
-- of them -- never a mix.
--
-- Three more fields travel with that set and are just as load-bearing locally:
--   * external_id -- the processor always writes it (billData.externalId = msg.billId),
--     and both admin actions address a bill by it: promote-bill 400s without a
--     'legiscan:'-prefixed value, and Re-generate POSTs externalId ?? id, so a NULL
--     here makes it post a local UUID that central cannot resolve. Every row carries
--     one, including the stub -- a real never-analyzed stub has an external_id too,
--     and without it the promote path this file exists to exercise cannot run at all.
--   * text_r2_key -- central supplies it together with text_status (processor.ts), and
--     hasBillText in BillDetail requires it, so text_status = 'in_r2' without a key
--     claims text is stored while hiding the bill-text section. Set on every 'in_r2'
--     row, keyed the way central keys legiscan docs: bills/legiscan-<id>/texts/<doc>.pdf.
--   * last_ai_text_doc_id -- drives the "based on text" chip beside the AI summary. Set
--     to a doc_id that actually exists in the bill_texts rows below (every analyzed bill
--     has at least one).
--
-- All rows below except the deliberate 'bill-stub-nomatch' lightweight stub are fully
-- analyzed. That row has match_type/ai_processed_at/tenant_summary/relevance_score/
-- text_status/text_r2_key/last_ai_text_doc_id all NULL and no bill_texts rows,
...
INSERT OR IGNORE INTO bill_texts (id, bill_id, doc_id, type, date, mime, text_hash, state_link) VALUES
  ...
  ('bt-12', 'bill-rcv', 10012, 'Introduced', '2026-01-18', 'application/pdf', 'hash12', NULL),
  ('bt-13', 'bill-access', 10013, 'Introduced', '2026-02-10', 'application/pdf', 'hash13', NULL),
  ('bt-14', 'bill-edr', 10014, 'Introduced', '2026-03-01', 'application/pdf', 'hash14', NULL),
  ('bt-15', 'bill-redistrict', 10015, 'Introduced', '2026-01-05', 'application/pdf', 'hash15', NULL),
  ('bt-16', 'bill-rolls', 10016, 'Introduced', '2026-01-08', 'application/pdf', 'hash16', NULL);
