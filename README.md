...
Bill data comes from [LegiScan](https://legiscan.com), licensed [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/); summaries and relevance scores are generated by FloorVote. FloorVote reads legislative data through a provider interface, and LegiScan is the maintained implementation.

FloorVote is open source under [AGPL-3.0](LICENSE). © 2026 William T. Adler and FloorVote contributors. Contributions are accepted under the [Developer Certificate of Origin](DCO) — sign your commits with `git commit -s`. If you run a modified version as a network service, the AGPL requires you to offer your users the corresponding source.
