...
You can change the model if you like — see [Choosing the analysis model](/self-hosting/tenants#choosing-the-analysis-model) for the two vars and when a newer model earns its cost. Google publishes [the model names](https://ai.google.dev/gemini-api/docs/models), and Cloudflare's [model catalog](https://developers.cloudflare.com/ai/models/) lists which ones you can reach through the AI Gateway and what each costs. Provider rates are passed through without markup; the only markup is the fee on [prepaid gateway credits](https://developers.cloudflare.com/ai-gateway/features/unified-billing/). A newer or larger model can cost several times as much per bill, so check the catalog before you switch.
