...
- **Path 1 (recommended — explore first).** Leave the AI fields and keywords blank, seed the session in Step 12, and every bill arrives as a free monitor stub with no AI cost. Browse and search the real corpus, then write your keywords and AI instructions here and press **Save keywords and sync**, which queues the newly-matching bills for full analysis. The free exploration window ends the moment you save — from then on, re-tuning keywords or instructions means paying for AI a second time on whatever newly matches.
...
> [!TIP]
> **No keywords yet is the expected case (Path 1 from Step 11).** Seeding uses whatever keywords are already synced to central to decide which bills get full AI versus a stub — with none set, every bill comes in as a stub, which is exactly the free-exploration state Path 1 wants. When you're ready to tune, set keywords and AI instructions in **Settings → Configuration** and save; changing them after seeding works too, but then you'd "Rerun AI on all bills" to regenerate against the new instructions.
