...
Because auto-login hands every visitor a session with no interaction, those writes are anonymous by construction, so two limits sit behind them: allowed writes are rate-limited per client IP (the optional `DEMO_WRITE_RATE_LIMITER` binding — see `api/wrangler.example.toml`; without it the check fails open), any one bill accepts at most 60 live comments, and any one comment carries at most 12 distinct reaction emojis (an emoji already there stays togglable). A visitor never meets any of them — the busiest seeded bill carries 8 comments and the reaction picker offers 8 emojis — but together they keep a script from growing the tenant, or filling a seeded comment with chips, between resets.
